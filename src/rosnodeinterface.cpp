  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"adder\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(adder_M, (NULL));
        adder_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(adder_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(adder_M)
                == (NULL)) || rtmGetStopRequested(adder_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(adder_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
